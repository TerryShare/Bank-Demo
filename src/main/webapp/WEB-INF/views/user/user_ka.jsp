...
<body class='contrast-red '>
		...
		<div class='navbar'>
			<div class='navbar-inner'>
				<div class='container-fluid'>
					<a class='brand' href='#'> <i class='icon-heart-empty'></i> <span
						class='hidden-phone'>个人网上银行系统</span>
					...
					<ul class='nav pull-right'>
						...
						<li class='dropdown dark user-menu'><a
							class='dropdown-toggle' data-toggle='dropdown' href='#'>  <span class='user-name hidden-phone'>${name }</span>
								<b class='caret'></b>
						</a>
							<ul class='dropdown-menu'>
								
								<li class='divider'></li>
								<li><a href='exit'> <i class='icon-signout'></i> Sign
										out
								</a></li>
							</ul></li>
					</ul>
	...
	<div id='wrapper'>
		...
		<nav class='' id='main-nav'>
			<div class='navigation'>
				...
				<ul class='nav nav-stacked'>
					...
					<li class='active'><a href='tzkp?name=${name}'> <i
		...
		<section id='content'>
						...
						<div class='row-fluid'>
							<div class='span12'>
								<div class='page-header'>
									<h1 class='pull-left'>
										<i class='icon-star'></i> <span>卡片管理</span>
									</h1>
			...
			<form action='lost?name=${name}' class='form form-horizontal validate-form' style='margin-bottom: 0;' />
            <div class='responsive-table'>
            	<ul class="nav nav-tabs" id="myTab">
					<li class="active"><a href="#debit" data-toggle="tab">储蓄卡</a></li>
				...
				<div class=' tab-pane active' id='debit'>
	                <div class='scrollable-area' >
	                    <table class='table' style='margin-bottom:0;'>
	                          <thead>
	                        <tr>
	                            <td><a href="" class="white-text templatemo-sort-by">卡号</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">激活时间</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">卡内余额</a></td>
	                          	<td><a href="" class="white-text templatemo-sort-by">挂失</a></td>
	                        ...
	                        <tbody id="list">
										<c:forEach items="${list}" var="list" varStatus="num">
											...
											<tr>
												<td id="cardid">${list.cardid}</td>
												<td id="registDate">${list.registDate}</td>
												<td id="money">${list.money}</td>
												<c:if test='${list.isLive == "true"}'>		
												<td id="isLost"><a class='btn' href='lost.do?id=${list.cardid}'>申请挂失</a></td>
												</c:if>	
												<c:if test='${list.isLive == "false"}'>		
												<td id="isLost">已申请挂失</td>
												</c:if>	
											</tr>
											</c:if>
										</c:forEach>
                ...
                <div class='tab-pane' id='credit'>
	                 <div class='scrollable-area ' >
	                    <table class='table' style='margin-botton:0;'>
	                          <thead>
	                        <tr>
	                            <td><a href="" class="white-text templatemo-sort-by">卡号</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">状态</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">激活时间</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">信用剩余</a></td>
	                            <td><a href="" class="white-text templatemo-sort-by">挂失</a></td>
	                         ...
	                         <tbody id="list">
										<c:forEach items="${list}" var="list" varStatus="num">
											...
											<tr>
												<td id="cardid">${list.cardid}</td>
												<c:if test='${list.isNew == "0"}'> 
													<td>未审核</td>
												</c:if>
												<c:if test='${list.isNew == "1"}'> 
													<td>已审核</td>
												</c:if>
												<td id="phone">${list.registDate}</td>
												<td id="address">${list.money}</td>
												<c:if test='${list.isLive == "true"}'>		
												<td id="isLost"><a class='btn' href='lost.do?id=${list.cardid}'>申请挂失</a></td>
												</c:if>	
												<c:if test='${list.isLive == "false"}'>		
